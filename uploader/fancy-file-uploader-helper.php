		...
		public static function HandleUpload($filekey, $options = array())
		{
			...
			else
			{
				...
				if ($name !== false)
				{
					...
					{
						if (file_exists($filename) && $startpos === filesize($filename)) {
					...
					else
					{
                        if ( file_exists( $filename ) ) {
                            ...
                            while( file_exists( $new_filename ) ) {
                                $index ++;
                                $new_filename = str_replace( $name, $name . "-" . strval( $index ), $filename );
                            }

                            $filename         = $new_filename;
                            $files[0]["name"] = str_replace( $name, $name . "-" . strval( $index ), $files[0]["name"] );
                        }
